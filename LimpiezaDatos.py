...
df_transacciones["plazo_dias"] = df_transacciones["plazo_dias"].str.replace("-","",regex=False)
...
df_transacciones["monto"] = df_transacciones["monto"].replace("Sin dato", pd.NA)
df_transacciones["monto"] = df_transacciones["monto"].replace("sin dato", pd.NA)
df_transacciones["monto"] = df_transacciones["monto"].fillna("0")

# print(df_transacciones["moneda"].unique())

# print(df_transacciones.loc[
#     df_transacciones["moneda"] == "Sin categoría",
#     ["monto", "moneda"]
# ].head(50))

pesos = df_transacciones["moneda"].isin(["COP", "Sin categoría"])
dolares = df_transacciones["moneda"] == "USD"

df_transacciones.loc[pesos, "monto"] = (
    df_transacciones.loc[pesos, "monto"]
    .str.replace(".", "", regex=False)
    .str.replace(",", "", regex=False)
)

df_transacciones.loc[dolares, "monto"] = (
    df_transacciones.loc[dolares, "monto"]
    .str.replace(",", "", regex=False)
)
...
df_transacciones.loc[dolares, "monto_COP"] = (
    df_transacciones.loc[dolares, "monto"] * dolar
)
...
df_clientes["nombre"] = df_clientes["nombre"].str.capitalize()
...
df_limpio["numero_documento"] = df_limpio["numero_documento"].fillna("Sin numero de documento")
df_limpio["tipo_documento"] = df_limpio["tipo_documento"].fillna("Sin tipo documento")
df_limpio["nombre"] = df_limpio["nombre"].fillna("Sin cliente")
df_limpio["ciudad"] = df_limpio["ciudad"].fillna("Sin ciudad")
df_limpio["segmento"] = df_limpio["segmento"].fillna("Sin segmento")
df_limpio["contacto"] = df_limpio["contacto"].fillna("Sin contacto")
df_limpio["productos"] = df_limpio["productos"].fillna("Sin productos")
df_limpio["activo"] = df_limpio["activo"].fillna("Sin estado")
df_limpio["plazo_dias"] = df_limpio["plazo_dias"].fillna(0)
df_limpio["plazo_dias"] = df_limpio["plazo_dias"].replace("",0)
